...
cat "$DIR"/senate.svg \
  | perl -p -e 's/class="geography"/opacity="0"/' \
  | perl -p -e 's/\<path class="[A-Z][A-Z].?.?"/<path fill="black" stroke="white" stroke-width="1"/g' \
  | perl -p -e 's/\<path class="mesh"/<path fill="black" stroke="white" stroke-width="20"/g' \
  | perl -p -e 's/<text /<text opacity="0" /g' \
  | rsvg-convert -w 647 -h 400 -b white \
  | convert - -monochrome "$DIR"/senate-cartogram-thumbnail.png
...
cat "$DIR"/senate.svg \
  ...
  | perl -p -e 's/<text /<text opacity="0" /g' \
  | rsvg-convert -w 647 -h 400 -b white \
  | convert - -monochrome "$DIR"/senate-geography-thumbnail.png
